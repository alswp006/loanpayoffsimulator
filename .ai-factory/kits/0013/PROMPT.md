## Task: S5 상세 스케줄(/schedule) 구현 + 리워드 게이트 + 가상 스크롤
전략별 월별 스케줄 표를 구현해요. 상단 TabBar로 전략을 전환하고, 스케줄 표 영역은 TossRewardAd로 감싸 잠금 해제 후에만 표시해요. 120행 초과 시 react-window FixedSizeList로 가상 스크롤을 적용하고, run 만료/전략 오류는 표 대신 안내를 보여요.

## Acceptance Criteria
1. IF runId가 없거나 NOT_FOUND이면 THEN "결과가 만료되었어요" 또는 오류 안내가 표시되고 "시뮬레이션으로" CTA가 렌더된다 (AC-S5-4)
2. IF 선택 strategy의 summary.status==='error'이면 THEN 스케줄 표 대신 오류 안내가 표시되고 표(ListRow 목록)는 렌더되지 않는다 (AC-S5-5)
3. WHEN rows.length > 120이면 THEN FixedSizeList(react-window)가 렌더되고, rows.length <= 120이면 THEN 일반 map 렌더가 사용된다 (AC-S5-6)
4. WHEN 리워드 광고 완료로 잠금 해제되면 THEN rewardUnlocksStore.unlockedRunIds에 runId가 포함되고, 이후 재진입 시 광고 없이 표가 표시된다 (AC-S5-1, AC-S5-2)

## Definition of Done
1. TossRewardAd로 표 영역 게이트(프로젝트 로컬 컴포넌트 시그니처 준수)
2. 전략 탭 전환 시 URL 이동 없이 내부 상태로 전환하되, 최초 진입 state.strategy를 초기값으로 사용
3. 120행 초과 가상 스크롤 적용
4. 오류/만료/빈 상태 문구와 CTA 제공

## Files to create/modify
src/pages/Schedule.tsx

## UI Requirements (TDS 컴포넌트 사용)
TDS 컴포넌트: Top, TabBar(전략 전환), ListRow(행), Spacing, Paragraph.Text, Button, Toast + (로컬) TossRewardAd
...
- Packet 0008: "상세 스케줄 생성기(런타임) + 월별 행 구조" (files: src/lib/simulation/generateSchedule.ts)
