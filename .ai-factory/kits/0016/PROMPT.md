## Task: 전역 내비게이션(TabBar) + Safe-area 폴리시/UX 폴리시 마감
앱 하단에 TDS TabBar 기반의 메인 내비(대출/설정)를 추가하고, 하단 fixed 영역에 safe-area-inset-bottom 패딩을 적용해 토스 앱 하단과 겹치지 않게 해요. 또한 공통적으로 버튼 터치 시 햅틱을 적용할 수 있도록 App 레벨 레이아웃 가이드를 마감해요(페이지 파일은 수정하지 않음).
...
## Definition of Done
1. TabBar는 2개 탭(대출, 설정)으로 구성되고 모바일 터치 타겟 44px 충족
2. Safe area 패딩이 포함된 fixed 하단 레이아웃 적용
3. 기존 페이지 파일과 충돌 없이 동작

## Files to create/modify
src/components/MainTabBar.tsx, src/App.tsx

## UI Requirements (TDS 컴포넌트 사용)
전역 UI(TDS): TabBar, Spacing(필요 시)
...
- Packet 0015: "라우팅/Provider Wiring (App.tsx)" (files: src/App.tsx)
