## Task: S6 설정/도움말(/settings) 구현 (전략/입력 가이드 BottomSheet)
설정/도움말 정적 화면을 구현해요. ListRow를 눌러 BottomSheet로 Snowball/Avalanche 차이, 입력값 범위, "대출 2개 이상" 제약을 안내해요. 외부 링크는 사용하지 않고, 필요한 경우 Toast로 간단 안내만 제공해요.

## Acceptance Criteria
1. WHEN /settings 진입 시 THEN 로딩 UI 없이 즉시 ListRow 목록이 렌더된다 (정적 화면)
2. WHEN "전략 설명" ListRow를 탭하면 THEN BottomSheet가 열리고 Snowball=잔액 작은 순, Avalanche=금리 높은 순 설명 문구가 포함된다
3. WHEN "입력 가이드" ListRow를 탭하면 THEN BottomSheet가 열리고 금리 0~30, 개월 1~600, 그리고 "대출 2개 이상" 제약 문구가 포함된다
4. IF 사용자가 BottomSheet를 닫으면 THEN 화면 내 라우팅 변화 없이 onClose로 닫히고 다시 열 수 있다

## Definition of Done
1. TDS Top/ListRow/BottomSheet/Paragraph.Text/Spacing만으로 구성
2. 외부 링크/설치 유도 문구 없음
3. 전략/입력 가이드 문구가 SPEC과 일치

## Files to create/modify
src/pages/Settings.tsx

## UI Requirements (TDS 컴포넌트 사용)
TDS 컴포넌트: Top, ListRow, Paragraph.Text, Spacing, BottomSheet
