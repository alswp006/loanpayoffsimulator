## Task: S4 결과 요약(/result) 구현 + Expired/복사/AdSlot
runId로 저장된 SimulationRun을 로드해 Snowball/Avalanche 요약을 비교 렌더링해요. runId 누락/미존재 시 Error/Expired 상태 문구와 "시뮬레이션으로" CTA를 제공해요. 전략이 error인 경우 해당 전략의 상세 버튼을 disabled 처리하고, 요약 섹션과 상세 버튼 섹션 사이에 AdSlot을 배치해요.

## Acceptance Criteria
1. IF location.state.runId가 없거나 runsStore.getRunById가 NOT_FOUND이면 THEN "결과가 만료되었어요"(Expired 케이스) 또는 일반 오류 안내가 표시되고 "시뮬레이션으로" 버튼 탭 시 navigate('/simulate')가 호출된다 (AC-S4-2, AC-S4-3)
2. WHILE run 로드 중이면 THEN "불러오는 중" 텍스트가 표시되고 주요 버튼이 disabled=true다 (AC-S4-1)
3. IF summaries[strategy].status==='error'이면 THEN 해당 전략 영역에 "계산 실패" 라벨과 errorCode 설명이 표시되고 "상세 스케줄 보기"는 disabled=true다 (AC-S4-4)
4. WHEN "결과 복사"를 탭하면 THEN 클립보드에 전략별 총이자/상환개월/절감액(가능한 범위) 텍스트가 1회 writeText 된다 (AC-S4-5)

## Definition of Done
1. 요약 정보(총이자/기간/절감액)와 전략 상태(ok/error)를 구분 렌더
2. AdSlot이 요약과 상세 버튼 사이에 1회 렌더
3. Expired 케이스 문구+CTA 제공
4. 상세 이동 navigate('/schedule',{state:{runId,strategy}}) 구현(단 error 전략은 비활성)

## Files to create/modify
src/pages/Result.tsx

## UI Requirements (TDS 컴포넌트 사용)
TDS 컴포넌트: Top, Paragraph.Text, ListRow, Button, Spacing, Chip, Toast, BottomSheet + (로컬) AdSlot
