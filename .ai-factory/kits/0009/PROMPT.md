## Task: S1 홈(/) 대출 목록 페이지 구현
대출 목록/홈 화면을 구현해요. hydrate 로딩/empty/1개 가이드/파싱 에러(AlertDialog+초기화)/삭제 확인 다이얼로그/시뮬레이션 CTA disabled 규칙을 충족하고, 목록 하단에 AdSlot 배너를 배치해요. ListRow 탭 시 /loan/edit로 state.loanId를 전달해요.

## Acceptance Criteria
1. WHILE loansStore.isHydrating가 true이면 THEN "불러오는 중" 텍스트가 DOM에 있고 "대출 추가" 버튼과 "시뮬레이션 시작" 버튼이 렌더되면 disabled=true다 (AC-S1-1)
2. WHEN loans.length===0이면 THEN "대출 추가" 버튼은 보이고 대출 ListRow는 0개다 (AC-S1-2)
3. IF loans.length<2이면 THEN "시뮬레이션 시작" 버튼은 disabled=true이고 loans.length===1일 때 안내 문구가 표시된다 (AC-S1-3)
4. IF hydrateErrorCode==='PARSE_ERROR'이면 THEN AlertDialog 제목이 "데이터를 불러올 수 없어요"로 표시되고 "초기화" 탭 후 lps_loans_v1이 빈 스키마로 재설정된다 (AC-S1-4)
5. WHEN ListRow를 탭하면 THEN navigate('/loan/edit',{state:{loanId}})가 1회 호출된다 (AC-S1-5)

## Definition of Done
1. TDS 컴포넌트만 사용(Spacing으로만 간격)
2. AdSlot이 목록 섹션 아래에 1회 렌더
3. 삭제 플로우(AlertDialog 확인→삭제→Toast) 동작
4. items<2 시 시뮬레이션 CTA disabled 유지
...
ListRow:
  ...
  ListRow.Texts (type prop REQUIRED — 21개 변형 존재):
    ...
    3줄형: type="3RowTypeA" | "3RowTypeB" | ... | "3RowTypeF" + top + middle + bottom
    오른쪽 정렬형: type="Right1RowTypeA~E" | "Right2RowTypeA~E"
      (string literal 값에는 "Texts" 접두어 없음. d.ts의 interface명만 TextsRight*)
    가장 흔한 사용:
      "1RowTypeA" — 단순 텍스트 1줄
      "2RowTypeA" — 제목 + 설명 (기본)
      "Right2RowTypeA" — 양쪽 정렬 (왼쪽 라벨 + 오른쪽 값)
